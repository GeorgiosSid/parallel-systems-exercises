...
void gaussian_blur_omp_loops(int radius, img_t *imgin, img_t *imgout)
{
	...
	#pragma omp parallel for private(i, j, col, row, redSum, greenSum, blueSum, weightSum) shared(radius, width, height, imgin, imgout)
	for (i = 0; i < height; i++)
	{
		//#pragma omp parallel for  private(j, col, row, redSum, greenSum, blueSum, weightSum) shared(i, radius, width, height, imgin, imgout)
		for (j = 0; j < width ; j++) 
		{
			//#pragma omp parallel for private(col, row) shared(i, j, radius, width, height, imgin) reduction(+:redSum) reduction(+:greenSum) reduction(+:blueSum) reduction(+:weightSum)
			for (row = i-radius; row <= i + radius; row++)
			{
					//#pragma omp parallel for private(col) shared(i, j, radius, width, row, height, imgin) reduction(+:redSum) reduction(+:greenSum) reduction(+:blueSum) reduction(+:weightSum)
					for (col = j-radius; col <= j + radius; col++)
					{
						int x = clamp(col, 0, width-1);
						int y = clamp(row, 0, height-1);
...
void gaussian_blur_omp_tasks(int radius, img_t *imgin, img_t *imgout)
{
	...
	#pragma omp parallel
	{
		#pragma omp single // only the master create the tasks
		{
			for (i = 0; i < height; i++)
			{
				#pragma omp task firstprivate(i, weightSum, redSum, greenSum, blueSum) private(j, row, col) shared(imgin, imgout, radius, width)
				{
					for (j = 0; j < width ; j++)
					{
						for (row = i-radius; row <= i + radius; row++)
